...
class UserCoreModel extends Model
{
    ...
    public function add(array $aData)
    {
        ...
        $rStmt = Db::getInstance()->prepare('INSERT INTO' . Db::prefix('Members') . '(email, username, password, firstName, lastName, sex, matchSex, birthDate, active, ip, hashValidation, joinDate, lastActivity,ban)
            VALUES (:email, :username, :password, :firstName, :lastName, :sex, :matchSex, :birthDate, :active, :ip, :hashValidation, :joinDate, :lastActivity,:ban)');
		...
		$rStmt->bindValue(':ban', (!empty($aData['is_active']) ? 1 : 2), \PDO::PARAM_INT);
        $rStmt->execute();
